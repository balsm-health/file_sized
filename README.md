...
<p align="center">
  ...
  <a href="https://pub.dev/packages/file_sized">
    <img alt="Pub Package" src="https://img.shields.io/pub/v/file_sized">
  </a>
   <a href="https://pub.dev/packages/file_sized">
    <img alt="Pub Points" src="https://img.shields.io/pub/points/file_sized">
  </a>
  <br/>
...
- 🎯 **Mixed Units Support**: Create file sizes from multiple units with `FileSize.units`
...
- 📐 **Comparable Interface**: Implements `Comparable<FileSize>` for natural sorting
...
When you need to combine multiple units into a single file size, use the `FileSize.units` factory constructor. This is particularly useful for expressing sizes like "2 GB + 500 MB + 256 KB":

```dart
// Video file: 2 GB + 500 MB + 256 KB
final videoFile = FileSize.units(
  gb: 2,
  mb: 500,
  kb: 256,
);
...
final onlyMB = FileSize.units(mb: 500);
print(onlyMB == FileSize.mb(500)); // true
```
...
The package provides convenient extension methods on `num` (both `int` and `double`) for creating `FileSize` instances with a more readable syntax:
...
|-----------|-------------|---------|---------------|
| `.b` | Bytes | `1024.b` | `FileSize.b(1024)` |
| `.kb` | Kilobytes | `1.5.kb` | `FileSize.kb(1.5)` |
| `.mb` | Megabytes | `100.mb` | `FileSize.mb(100)` |
| `.gb` | Gigabytes | `2.5.gb` | `FileSize.gb(2.5)` |
| `.tb` | Terabytes | `1.tb` | `FileSize.tb(1)` |

**Note:** The `.b` extension automatically converts to integer (truncates decimals) since bytes must be whole numbers.

### Arithmetic Operations

The `FileSize` class supports arithmetic operations for calculating storage totals, quotas, and proportions.

#### Addition and Subtraction

```dart
// Adding file sizes
final document = FileSize.mb(2);
final spreadsheet = FileSize.kb(500);
final presentation = FileSize.kb(1);
final total = document + spreadsheet + presentation;
print(total.format()); // "2.49 MB"

// Subtracting file sizes
final totalStorage = FileSize.gb(1);
final usedSpace = FileSize.mb(750);
final available = totalStorage - usedSpace;
print(available.format()); // "274.00 MB"

// Result is clamped to zero if negative
final small = FileSize.kb(100);
...
`FileSize` implements `Comparable<FileSize>` for seamless sorting:
...
The `FileSize` class supports equality and comparison operations, making it easy to compare file sizes regardless of the units used to create them.
...
print(small <= FileSize.kb(500));  // true (equal)
print(medium >= FileSize.mb(0.5)); // true (greater)
...
bool isValidFileSize(FileSize fileSize, FileSize maxSize) {
  return fileSize <= maxSize;
}

// Storage management
void manageStorage(List<FileSize> files, FileSize availableSpace) {
  final totalSize = files.fold<FileSize>(
    FileSize.zero,
    (sum, file) => sum + file,
  );
...
List<FileSize> findLargeFiles(List<FileSize> files, FileSize threshold) {
...
- **Collection management** using FileSize with Set, Map, and sorting
...
| ------------------------- | ------------------------------- | ------------------- |
| `FileSize.zero`          | Static instance with zero bytes | `FileSize.zero`    |
| `FileSize.b(int bytes)`  | Creates instance from bytes     | `FileSize.b(1024)` |
| `FileSize.kb(num kb)` | Creates instance from kilobytes | `FileSize.kb(1.5)` |
| `FileSize.mb(num mb)` | Creates instance from megabytes | `FileSize.mb(100)` |
| `FileSize.gb(num gb)` | Creates instance from gigabytes | `FileSize.gb(2.5)` |
| `FileSize.tb(num tb)` | Creates instance from terabytes | `FileSize.tb(1)`   |
| `FileSize.units({...})` | Creates from multiple units     | `FileSize.units(gb: 2, mb: 500)` |
...
| ---------- | ------------------------- | -------------------------------------- |
| `==`       | Equality comparison       | `FileSize.kb(1) == FileSize.b(1024)` |
...
| Operator | Description           | Example                                   |
| -------- | --------------------- | ----------------------------------------- |
| `<`      | Less than             | `FileSize.kb(1) < FileSize.mb(1)`       |
| `<=`     | Less than or equal    | `FileSize.kb(1) <= FileSize.b(1024)`    |
| `>`      | Greater than          | `FileSize.mb(1) > FileSize.kb(1)`       |
| `>=`     | Greater than or equal | `FileSize.mb(1) >= FileSize.b(1048576)` |

#### Arithmetic Operators

| Operator | Description                | Returns     | Example                               |
| -------- | -------------------------- | ----------- | ------------------------------------- |
| `+`      | Addition                   | `FileSize` | `FileSize.mb(1) + FileSize.kb(500)` |
| `-`      | Subtraction (clamped to 0) | `FileSize` | `FileSize.mb(2) - FileSize.kb(500)` |
| `*`      | Multiplication by scalar   | `FileSize` | `FileSize.mb(10) * 3`                |
| `/`      | Division by scalar         | `FileSize` | `FileSize.mb(30) / 3`                |

#### Other Methods

| Method        | Description                     | Example                                            |
| ------------- | ------------------------------- | -------------------------------------------------- |
| `toString()`  | String representation           | `FileSize.mb(1.5).toString()` returns `"1.50 MB"` |
...
| Method                    | Description                  | Example                                    |
| ------------------------- | ---------------------------- | ------------------------------------------ |
| `min(sizes)`              | Returns smallest size from collection | `FileSize.min([size1, size2, size3])`     |
| `max(sizes)`              | Returns largest size from collection | `FileSize.max([size1, size2, size3])`     |
| `sum(sizes)`              | Sum of all sizes             | `FileSize.sum([size1, size2, size3])`     |
| `average(sizes)`          | Average of all sizes         | `FileSize.average([size1, size2, size3])` |
| `setPostfixesGenerator()` | Set global postfix generator | `FileSize.setPostfixesGenerator(fn)`      |
